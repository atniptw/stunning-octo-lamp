...
export async function fixPRCommand(prNumber: string) {
  ...
  try {
    ...
    console.log(
      chalk.dim(
        `${actionItems.length} items need attention (ordered by priority)`,
      ),
    );
    ...
    actionItems.forEach((item, index) => {
      ...
      if (item.type === "status_check") {
        ...
        console.log(
          chalk.dim('     2. git add . && git commit -m "Fix: <description>"'),
        );
        console.log(chalk.dim("     3. git push"));
        console.log(
          chalk.dim("     4. workflow check-pr " + prNum + " (verify fix)"),
        );
      } else if (item.type === "comment_response") {
        console.log(chalk.dim("     1. Address the feedback locally"));
        console.log(
          chalk.dim(
            '     2. git add . && git commit -m "Address comment: <description>"',
          ),
        );
        console.log(chalk.dim("     3. git push"));
        console.log(
          chalk.dim(
            `     4. workflow reply-comment ${prNum} ${item.commentId} "<commit-hash>"`,
          ),
        );
        ...
        console.log(
          chalk.dim(
            '     2. git add . && git commit -m "Address review: <description>"',
          ),
        );
        console.log(chalk.dim("     3. git push"));
    ...
    if (criticalCount > 0) {
      console.log(
        chalk.red(`  🚨 ${criticalCount} critical issue(s) blocking merge`),
      );
    }
    ...
    console.log(
      ...
      prNum,
      chalk.dim("- Refresh action items"),
    );
    console.log(
      chalk.yellow("  workflow check-pr"),
      prNum,
      chalk.dim("- View detailed status"),
    );
    console.log(
      chalk.yellow("  workflow comment-pr"),
      prNum,
      '"message"',
      chalk.dim("- Add general comment"),
    );
    console.log(
      chalk.yellow("  workflow push-changes"),
      chalk.dim("- Commit and push current changes"),
    );
...
function getPriorityDisplay(priority: number): string {
  switch (priority) {
    ...
    case 3:
      return chalk.yellow.bold("⚠️  HIGH");
    case 2:
