...
export async function createPRCommand(storyId: string) {
  ...
  try {
    ...
    if (
      ...
      console.log(
        chalk.blue("File:"),
        chalk.dim(`.github/pulls/${prNumber}.md`),
      );
      console.log(
        "\n" +
          chalk.yellow(
            "Note: This is a mock PR. Configure GitHub authentication to create real PRs.",
          ),
      );
    } else {
