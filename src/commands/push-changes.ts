...
export async function pushChangesCommand(message?: string) {
  ...
  try {
    ...
    console.log(chalk.yellow('  workflow check-pr <prNumber>'), chalk.dim('- Check if status checks pass'));
    console.log(chalk.yellow('  workflow reply-comment <prNumber> <commentId>'), chalk.green(commitHash), chalk.dim('- Reply to comment with commit'));
    ...
    if (error.message.includes('nothing to commit')) {
      ...
      console.log(chalk.dim('  git pull origin $(git rev-parse --abbrev-ref HEAD)'));
    }
...
async function generateCommitMessage(statusLines: string[]): Promise<string> {
  ...
  const hasTests = statusLines.some(line => line.includes('test') || line.includes('.spec.') || line.includes('.test.'));
  const hasDocs = statusLines.some(line => line.includes('.md') || line.includes('doc'));
  const hasConfig = statusLines.some(line => 
    line.includes('.json') || line.includes('.yml') || line.includes('.yaml') || line.includes('.env')
  );
  const hasSource = statusLines.some(line => 
    line.includes('.ts') || line.includes('.js') || line.includes('.tsx') || line.includes('.jsx')
  );
