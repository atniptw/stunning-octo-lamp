...
export async function replyCommentCommand(prNumber: string, commentId: string, commitHash?: string) {
  ...
  try {
    ...
    if (!hash) {
      ...
      try {
        ...
        throw new Error('Could not get commit hash. Provide one explicitly or ensure git is available.');
      }
    }
    ...
    console.log(chalk.dim('  ' + (targetComment.body.length > 100 
    ...
    console.log(chalk.yellow('  workflow check-pr'), prNum, chalk.dim('- Check if status checks pass'));
    console.log(chalk.yellow('  workflow fix-pr'), prNum, chalk.dim('- See remaining action items'));
    console.log(chalk.blue('  Web:'), prDetails.html_url, chalk.dim('- View on GitHub'));
...
function generateReplyMessage(comment: any, commitHash: string, prDetails: any): string {
