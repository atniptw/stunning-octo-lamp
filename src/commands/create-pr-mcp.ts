...
export async function createPRMCPCommand(storyId: string) {
  ...
  try {
    ...
    const { stdout: commits } = await execAsync(`git log origin/main..HEAD --oneline`);
    ...
    if (!commits.trim()) {
      ...
      console.log(chalk.dim('2. Push your branch: git push -u origin ' + branch));
    ...
    try {
      ...
      console.log(chalk.yellow('Push your branch first: git push -u origin ' + branch));
      process.exit(1);
    }
