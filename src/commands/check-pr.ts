...
export async function checkPRCommand(prNumber: string) {
  ...
  try {
    ...
    const prDetails = await (service as GitHubAppService).getPullRequestDetails(prNum);
    
    // Get status information
    spinner.text = 'Checking status and reviews...';
    const status = await (service as GitHubAppService).checkPullRequestStatus(prNum);
    
    // Get comments
    spinner.text = 'Loading comments...';
    const comments = await (service as GitHubAppService).getPullRequestComments(prNum);
    ...
    console.log(chalk.blue('State:'), getStateColor(prDetails.state)(prDetails.state.toUpperCase()));
    console.log(chalk.blue('Author:'), prDetails.user.login);
    console.log(chalk.blue('URL:'), chalk.green(prDetails.html_url));
    console.log(chalk.blue('Created:'), new Date(prDetails.created_at).toLocaleString());
    console.log(chalk.blue('Updated:'), new Date(prDetails.updated_at).toLocaleString());
    console.log(chalk.blue('Branches:'), `${prDetails.head.ref} → ${prDetails.base.ref}`);

    // Merge status
    console.log('\n' + chalk.bold('🔀 Merge Status:'));
    console.log(chalk.blue('Mergeable:'), getMergeableColor(status.mergeable)(
      status.mergeable === null ? 'UNKNOWN' : 
      status.mergeable ? 'YES' : 'NO'
    ));
    console.log(chalk.blue('Merge State:'), getMergeStateColor(status.mergeable_state)(status.mergeable_state));
    ...
    if (status.checks.length === 0) {
      ...
      status.checks.forEach(check => {
        ...
        console.log(`    Status: ${statusColor(check.status)} ${check.conclusion ? `(${check.conclusion})` : ''}`);
    ...
    if (status.reviews.length === 0) {
      ...
      status.reviews.forEach(review => {
        ...
        console.log(`  ${reviewIcon} ${chalk.bold(review.user)} ${reviewColor(review.state)}`);
        if (review.submitted_at) {
          console.log(`    ${chalk.dim(new Date(review.submitted_at).toLocaleString())}`);
        }
      });
    }
    ...
    if (comments.length === 0) {
      ...
      recentComments.forEach(comment => {
        const commentType = comment.is_review_comment ? '📝 Code' : '💬 General';
    ...
    const pendingChecks = status.checks.filter(c => c.status === 'in_progress' || c.status === 'queued');
    ...
    if (pendingChecks.length > 0) {
      console.log(chalk.yellow(`  ⏳ ${pendingChecks.length} pending check(s)`));
    ...
    console.log(chalk.yellow('  workflow check-pr'), prNum, chalk.dim('- Refresh status'));
    console.log(chalk.yellow('  workflow comment-pr'), prNum, '"message"', chalk.dim('- Add comment'));
...
function getMergeStateColor(state: string) {
  switch (state) {
    ...
    case 'dirty': return chalk.red;
    case 'unstable': return chalk.yellow;
...
function getCheckColor(status: string, conclusion: string | null) {
  if (status === 'completed') {
    switch (conclusion) {
      ...
      case 'skipped': return chalk.blue;
      default: return chalk.yellow;
    }
  }
  return chalk.yellow;
...
function getReviewColor(state: string) {
  switch (state) {
    ...
    case 'COMMENTED': return chalk.blue;
    default: return chalk.gray;
  }
}
